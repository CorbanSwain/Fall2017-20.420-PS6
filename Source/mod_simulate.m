function [dT, dX, dY, vT, vX, vY, vB] = mod_simulate(simTime, atpConc, dt0, ...
...
vT = repmat(timeVector, 1, nVesicle);
[vX, vY, vB] = deal(zeros(size(vT)));
...
%     fprintf("\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b");
    ...
    for iVes = 1:nVesicle
        ...
        vX(iTime, iVes) = pos(1);
        vY(iTime, iVes) = pos(2);
        vB(iTime, iVes) = vesicles(iVes).isbound;
    end
...
if dyneinPresent
    ...
    for iDyn = 1:nDynein
        ...
        dT{iDyn} = tempDT(1:uCount, iDyn);
        dX{iDyn} = tempDX(1:uCount, iDyn);
        dY{iDyn} = ones(uCount, 1) .* dInitialPos(iDyn, 2);
    end
end
